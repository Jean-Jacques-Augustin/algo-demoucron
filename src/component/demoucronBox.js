...
function DemoncronBox() {
  ...
  let trouveSolution = async () => {
    ...
    if (typeChemin == "min") {
      ...
      var matrice = grapheRef.current.props.recuperationMatrice(
      ...
      <Container>
        ...
        <div className="nombreSommetContainer">
          ...
          <input
            type="number"
            value={nombreSommet}
            onChange={(e) => setNombreSommet(e.target.value)}
            min="2"
          ></input>
    ...
    <>
      ...
      {props.demarche.map((etape, index) => (
        <div key={index}>
          <MatriceD
            k={etape.k}
            lastMatrice={etape.lastMatrice}
            currentMatrice={etape.currentMatrice}
            a_changee={etape.a_changee}
            w={etape.w}
            entree={etape.entree}
            sortie={etape.sortie}
            typeChemin={props.typeChemin}
          />
          <br />
        </div>
      ))}
