...
function MatriceD(props){
    ...
    return(
        ...
        <Card.Title>k = {props.k + 1}</Card.Title>
            <Row>
                <Col>
                    <h4 className="titleMatrice">Matrice D<sub>{props.k}</sub></h4>
                ...
                <Col>
                <h4 className="titleMatrice">Matrice D<sub>{props.k+1}</sub></h4>
                <table className='matrice'>
                <tbody>
                <tr className='matriceTopNumero'>
                                <td></td>
                        ...
                        entree.map((i)=>(
                            sortie.map((j, index)=>(
                                <tr key={index} className="formule">
                                    ...
                                    <td>
                                        V<sub>{(i+1)+""+(j+1)}</sub><sup>{"("+(k+1)+")"}</sup>= {props.typeChemin.toUpperCase()} {"( W"}<sub>{(i+1)+""+(j+1)}</sub><sup>{"("+k+")"}</sup>, V<sub>{(i+1)+""+(j+1)}</sub><sup>{"("+(k)+")"}</sup>{" )"}= {props.typeChemin.toUpperCase()}{"( "+w[i][j]+","} <>{(!Number.isFinite(lastMatrice[i][j])? <>{lastMatrice[i][j]==Infinity ? <>+</> : <>-</>}&infin;</> :lastMatrice[i][j])} {" )="}<b className="a_changee">{currentMatrice[i][j]}</b></>
                                    </td>
                                </tr>
